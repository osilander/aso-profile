...
if [[ "${SKIP_RNAFOLD:-0}" == "1" ]]; then
  echo "SKIP_RNAFOLD=1 — skipping (using existing RNAfold output files)"
  for f in \
    "${RNAFOLD_DIR}/pathogenic_missense.plusminus100.rnafold.txt" \
    "${RNAFOLD_DIR}/common_snps.plusminus100.rnafold.txt"; do
    [[ -s "$f" ]] || { echo "ERROR: missing expected RNAfold output: $f" >&2; exit 1; }
  done
else
